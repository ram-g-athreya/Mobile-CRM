...
rules <- apriori(transactions, 
                 parameter=list(support=0.00000001, confidence=0.05)#,
                 #appearance=list(lhs=c("1=1","2=1","3=1","4=1","5=1"),default="lhs")
                 );
print(proc.time() - ptm);

#rules <- eclat(transactions, parameter=list(support=0.00000001));
#rules <- ruleInduction(rules, transactions, control=list(verbose=TRUE));
