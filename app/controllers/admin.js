exports.index = function(req, res) {
    if (req.user) {
        app.db.models.tbl_product.count({
            is_sold: 0
        }, function(err, product_count) {
                ...
                (function process(index) {
                    if (index < limit) {
                        app.db.driver.execQuery("call getCustomerPurchases(" + customer[index].id_customer + ");", function(err, data) {
                            ...
                            if (data.brands) {
                                app.db.models.tbl_sold_rules.find(['rhs'], {
                                    lhs: data.brands
                                }, {
                                    limit: 1
                                }, ["support", "Z"], function(err, rules) {
                                    if (err) {
                                        throw err;
                                    }

                                    rules = rules[0];
                                    results[index] = "Customer " + customer[index].id_customer + " has already bought " + data.brand_types + ". So he will be recommended a/an " + brand_types[rules.rhs - 1].brand_type;
                                    process(++index);
                                });
                            }
                            //No Purchases made then most bought product is recommended
                                ...
                                app.db.models.tbl_sold_rules.find(['rhs'], {}, {
                                    ...
                                    rules = rules[0];
                                    results[index] = "Customer " + customer[index].id_customer + " has not bought anything" + ". So he will be recommended the most bought product which is " + brand_types[rules.rhs - 1].brand_type;
                                    process(++index);
                                });
                            }
                        });
                        ...
                        if (page) {
                            res.render('templates/admin/recommend', {
                                data: results
                            });
                        } else {
